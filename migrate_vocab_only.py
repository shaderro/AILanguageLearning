...
# 兼容在任意位置运行：把项目根目录加入 sys.path
PROJECT_ROOT = Path(__file__).resolve().parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

# 导入 ORM 模型（基于 data_classes_new 对齐）
...
# 词汇 JSON 路径
VOCAB_JSON = PROJECT_ROOT / "backend/data/current/vocab.json"
...
def migrate_vocab():
    ...
    if not VOCAB_JSON.exists():
        raise FileNotFoundError(f"JSON not found: {VOCAB_JSON}")
