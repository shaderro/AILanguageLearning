...
根据项目中的 `scripts/sql/add_invite_codes_to_postgres.sql` 和 `scripts/sql/add_invite_codes_to_postgres_safe.sql` 脚本，**初始创建的一批邀请码**为：
...
项目 `scripts/admin/` 目录下有 `list_invite_codes.py`，可快速列出所有邀请码及使用状态：

```bash
# 连接线上库
ENV=production python scripts/admin/list_invite_codes.py
...
可修改 `scripts/admin/generate_invite_codes.py` 末尾：
...
```bash
ENV=production python scripts/admin/view_all_users.py
```
...
# Linux / macOS
ENV=production python scripts/admin/generate_invite_codes.py
```
...
| 操作                 | 方式 / 命令 |
|----------------------|-------------|
| 添加新邀请码         | `python scripts/admin/generate_invite_codes.py` 或 SQL `INSERT` |
| 查看所有邀请码及状态 | `ENV=production python scripts/admin/list_invite_codes.py` |
...
| 查看用户列表         | `python scripts/admin/view_all_users.py`（设置 `ENV` 可查生产库） |
