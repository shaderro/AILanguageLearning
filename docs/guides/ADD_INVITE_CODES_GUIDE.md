...
2. **执行脚本**
   ...
   - 选择 `scripts/sql/add_invite_codes_to_postgres.sql` 文件
   - 点击 `Execute`（或按 `F5`）
...
### 1. `scripts/sql/add_invite_codes_to_postgres.sql`（简单版本）
...
### 2. `scripts/sql/add_invite_codes_to_postgres_safe.sql`（安全版本）
