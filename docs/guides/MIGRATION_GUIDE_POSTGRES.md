...
3. **执行迁移脚本**
   ```sql
   \i scripts/sql/migrate_token_system_postgres_complete.sql
   ```
   或者在 pgAdmin 中：
   - 打开 `scripts/sql/migrate_token_system_postgres_complete.sql` 文件
...
# 运行迁移脚本
python scripts/migrations/migrate_add_invite_token_system.py
python scripts/migrations/migrate_add_token_logs_table.py
python scripts/migrations/migrate_add_assistant_name_to_token_logs.py
```

## 迁移脚本说明

### `scripts/sql/migrate_token_system_postgres_complete.sql`
...
- `scripts/sql/migrate_token_system_postgres_complete.sql` - 完整的 PostgreSQL 迁移脚本
