...
class DataBindingService(EventDispatcher):
    ...
    def register_viewmodel(self, name: str, viewmodel: 'BaseViewModel') -> bool:
        ...
        try:
            ...
            print(f"DataBindingService: 注册ViewModel - {name}")
            return True
        except Exception as e:
            print(f"DataBindingService: 注册ViewModel失败 - {e}")
            return False
    ...
    def unregister_viewmodel(self, name: str) -> bool:
        ...
        try:
            if name in self._viewmodels:
                ...
                print(f"DataBindingService: 注销ViewModel - {name}")
                return True
            return False
        except Exception as e:
            print(f"DataBindingService: 注销ViewModel失败 - {e}")
            return False
    ...
    def bind_data_to_viewmodel(self, data_key: str, viewmodel_name: str, 
        ...
        try:
            if viewmodel_name not in self._viewmodels:
                print(f"DataBindingService: 错误 - ViewModel {viewmodel_name} 未注册")
                return False
            ...
            print(f"DataBindingService: 创建数据绑定 - {data_key} -> {viewmodel_name}.{property_name}")
            return True
        except Exception as e:
            print(f"DataBindingService: 创建数据绑定失败 - {e}")
            return False
    ...
    def unbind_data_from_viewmodel(self, data_key: str, viewmodel_name: str, property_name: str) -> bool:
        ...
        try:
            ...
            if binding_key in self._bindings:
                ...
                print(f"DataBindingService: 解绑数据 - {data_key} -> {viewmodel_name}.{property_name}")
                return True
            return False
        except Exception as e:
            print(f"DataBindingService: 解绑数据失败 - {e}")
            return False
    ...
    def update_data(self, data_key: str, new_value: Any, sync_immediately: bool = True) -> bool:
        ...
        try:
            ...
            print(f"DataBindingService: 更新数据 - {data_key}: {str(new_value)[:50]}...")
            return True
        except Exception as e:
            print(f"DataBindingService: 更新数据失败 - {e}")
            return False
    ...
    def sync_all_data(self) -> bool:
        ...
        try:
            ...
            print("DataBindingService: 所有数据同步完成")
            return True
        except Exception as e:
            print(f"DataBindingService: 数据同步失败 - {e}")
            return False
    ...
    def get_viewmodel(self, name: str) -> Optional['BaseViewModel']:
        ...
        Returns:
            BaseViewModel: ViewModel实例，如果不存在则返回None
        """
        return self._viewmodels.get(name)
    
    def get_all_viewmodels(self) -> Dict[str, 'BaseViewModel']:
        """获取所有注册的ViewModel
        
        Returns:
            Dict: 所有ViewModel的字典
        """
        return self._viewmodels.copy()
        ...
        for binding_key, binding in self._bindings.items():
            if binding['data_key'] == data_key:
                ...
                try:
                    ...
                    print(f"DataBindingService: 同步数据 - {data_key} -> {binding['viewmodel_name']}.{property_name}")
                except Exception as e:
                    print(f"DataBindingService: 同步数据失败 - {e}")
    ...
    def _cleanup_bindings_for_viewmodel(self, viewmodel_name: str) -> None:
        ...
        Args:
            viewmodel_name: ViewModel名称
        """
        binding_keys_to_remove = []
        for binding_key, binding in self._bindings.items():
            if binding['viewmodel_name'] == viewmodel_name:
                binding_keys_to_remove.append(binding_key)
        
        for key in binding_keys_to_remove:
            del self._bindings[key]
        
        if binding_keys_to_remove:
            print(f"DataBindingService: 清理了 {len(binding_keys_to_remove)} 个绑定关系") 